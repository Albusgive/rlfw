...
// 开启串口
bool Serial::OpenSerial(std::string SerialPort, int Bps, int DataSize,
                        int Parity, int StopBit) {
  // Close();
  nSerialID = open(SerialPort.c_str(), O_RDWR | O_NOCTTY | O_NONBLOCK);
  if (-1 == nSerialID) {
    /* 不能打开串口一*/
    std::string str = SerialPort + " open fail !!!";
    perror(str.c_str());
    return false;
  }
  struct termios Opt;
  ...
  auto it = baudrate_map.find(Bps);
  if (it == baudrate_map.end()) {
    std::cout << "Bps err" << std::endl;
    return false;
  }
  cfsetispeed(&Opt, it->second);
  cfsetospeed(&Opt, it->second);
  ...
  case 8:
    ...
    break;
  default:
    std::cout<<"DataSize err"<<std::endl;
    return false;
  }
  ...
  case 2:
    ...
    break;
  default:
    std::cout<<"StopBit err ,please 1 or 2"<<std::endl;
    return false;
  }

  // 如果只是串口传输数据，而不需要串口来处理，那么使用原始模式(Raw
  // Mode)方式来通讯，设置方式如下：
...
int Serial::Send(const std::vector<uint8_t>& buffer) {
  if (buffer.empty()) {
      return -1; // 空向量直接返回错误
  }
  // 检查 vector 大小是否超过 int 范围
...
void Serial::register_comport(std::list<std::string> &comList,
...
void Serial::probe_serial8250_comports(std::list<std::string> &comList,
                                       std::list<std::string> comList8250) {
  ...
  std::thread ThRecv = std::thread{[&]() {
    ...
    while (b_OpenSign) {
      ...
      if (res > 0) {
        is_recv.store(false);
        for (int i = 0; i < res; i++) {
          recv_queue.push_back((uint8_t)RecvBuf[i]);
...
void Serial::decode() {
  ...
  decode_lambda(recv_queue,this->name);
...
void Serial::connectDecode(std::function<void(std::vector<uint8_t>&,std::string&)> lambda) {
...
int Serial::RefreshBuffer(unsigned char *pBuf, int Len, bool RecvTypet) {
  ...
  if (false == RecvTypet) {
    ...
    if ((Len + nSum) <= RecvBufferLen) //	总长度小于1K
    {
      ...
      if (Len <=
          RecvBufferLen) //	拷贝满1K空间，丢弃掉aucT[0]开始的字符，并进行填充，!!!!!!!!!!!
      {
