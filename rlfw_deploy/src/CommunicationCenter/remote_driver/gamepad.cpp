...
bool GamePad::openRemote() {
  if (openGamePad(channel) == -1)
    return false;
  readGamePad();
  return true;
};
void GamePad::closeRemote() { unreadGamePad(); };
    ...
    if (node != NULL) {
      ...
      if (((std::string)node).find("js") != std::string::npos) {
        ...
        struct udev_device *parent =
            udev_device_get_parent_with_subsystem_devtype(dev_input,
                                                          "bluetooth", NULL);
        if (parent) {
          // 保存手柄
          InputDevice input;
          ...
          if (parent) {
            ...
            if (input.type != "ble") {
              ...
              if (str != NULL) {
                input.VID = (std::string)udev_device_get_sysattr_value(
                    parent, "idVendor");
              }
              // 上一级父设备，不然设备名搜不到,因为js和event是同一支个下两个分支
      ...
      if (js->parent_sysname == ev.parent_sysname) {
        ...
        if (ioctl(fd, EVIOCGNAME(sizeof(name) - 1), &name) < 0) {
          ...
          if (js->type == "ble") {
            ...
            GamePadpads.insert(
                std::pair<std::string, std::string>(js->path, description));
          }
          close(fd);
          break;
        }
      ...
      for (auto it : dev_js) {
        if (it.PID == Pid && it.VID == Vid) {
          std::string description(
              it.name + " | " + "USB DEVICE: " +
              (std::string)udev_device_get_sysattr_value(dev, "manufacturer") +
              "| " +
              (std::string)udev_device_get_sysattr_value(dev, "product"));
          GamePadpads.insert(
              std::pair<std::string, std::string>(it.path, description));
          break;
        }
      }
    }
...
void GamePad::showGamePads() {
  ...
  for (auto it : GamePadpads) {
    ...
    std::cout << "  ∟﹣﹣﹣详细信息："
              << it.second.substr(name + 2, it.second.size()) << std::endl;
  }
  std::cout << std::endl;
}
...
void GamePad::bindKeyValue(
    std::function<void(std::vector<std::string> &, std::vector<float> &)>
        key_value) {
  value.resize(20);
  this->bindGamePadValues([=](GamePadValues map) {
    value[0] = map.a;
    value[1] = map.b;
    value[2] = map.x;
    value[3] = map.y;
...
void GamePad::readGamePad() {
  ...
  std::thread read_thread(std::bind(&GamePad::Rec, this));
  read_thread.detach();
}
