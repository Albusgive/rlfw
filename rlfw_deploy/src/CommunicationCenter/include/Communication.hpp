...
public:
  ...
  bool check() {
    if (jointname == "") {
      std::cout << "no jointname" << std::endl;
      return false;
    }
    bool flag = true;
    flag = flag && (type != Motortype::ERR);
    if (!flag)
      std::cout << jointname + " type:" << magic_enum::enum_name(type)
                << std::endl;
    flag = flag && (ctrl_type != MotorCtrlType::ERR);
    if (!flag)
      std::cout << jointname + " ctrl_type:" << magic_enum::enum_name(ctrl_type)
                << std::endl;
    return flag;
  }
};
...
public:
  ...
  bool check() {
    ...
    case ComType::ERR: {
      std::cout << name + " Invalid communication type" << std::endl;
      flag = false;
      break;
    }
    }
